...
import { fetchGeneral, fetchPersonalised, fetchFollowing, fetchStatus, fetchOnboardingStatus } from './api'
...
const FOLLOWING_TOPICS = [
  ...
  'Hugging Face', 'LangChain', 'Mistral AI', 'Cursor', 'Railway',
]

function getStoredToken() {
  return sessionStorage.getItem('updayte_token')
}
      ...
      <div className="flex items-center gap-3">
        ...
        {articleCount != null && (
          <span className="text-xs text-zinc-500 hidden sm:block">
            {articleCount.toLocaleString()} articles in database
          </span>
        )}
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    if (!authed) { setOnboarded(null); return }
    fetchOnboardingStatus()
      .then(data => setOnboarded(data?.is_onboarded ?? false))
  ...
  // Load article count once on mount
  useEffect(() => {
    if (!authed || !onboarded) return
    fetchStatus().then(s => s?.article_count != null && setArticleCount(s.article_count))
  }, [authed, onboarded])

  // Reload whenever tab or topic changes
  useEffect(() => {
    if (!authed || !onboarded) return
    loadFeed(activeTab, activeTopic)
  }, [authed, onboarded, activeTab, activeTopic, loadFeed])
  ...
  // ── Main app ───────────────────────────────────────────────────────────────
  const feedTitle = activeTab === 'following'
    ? activeTopic
    : activeTab === 'personalised'
    ? 'For You'
    : 'General'

  const grouped = activeTab !== 'following'
      ...
      <div className="max-w-5xl mx-auto px-4 lg:px-6 py-6 flex gap-8">
        ...
        <main className="flex-1 min-w-0">
          ...
          <FeedList
            ...
            loading={loading}
            error={error}
            onFollowingClick={handleTopicClick}
            grouped={grouped}
          />
        </main>
      </div>
